...
//     	$id=0;
    	if($entity instanceof Disponibilite){
    		...
    		$dql = 'SELECT (count(d)) FROM AssoMakerPHPMBundle:Disponibilite d WHERE (d.orga = :pid) AND (d.debut < :fin ) AND (d.fin > :debut) AND (d.id!=:id )';
    		...
    		$dql = 'SELECT (count(p)) FROM AssoMakerPHPMBundle:PlageHoraire p WHERE p.tache = :pid AND (p.debut < :fin ) AND (p.fin > :debut) AND p.id!=:id';
