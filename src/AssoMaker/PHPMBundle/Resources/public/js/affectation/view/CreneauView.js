...
CreneauView.prototype = {
	...
	initialize: function() {
		// écoute des boutons
		$('#bouton_tache_refresh').off('click').on('click', function() { pmAffectation.controllers.creneau.getData(); });
		
		this.setFilters(); // met les bonnes valeurs dans les filtres
		
		// filtres : bind les events
		$('#filtre_tache_confiance').off('change').on('change', function() { pmAffectation.controllers.creneau.clickFilter('confiance', $('#filtre_tache_confiance').val()); });
		$('#filtre_tache_equipe').off('change').on('change', function() { pmAffectation.controllers.creneau.clickFilter('equipe', $('#filtre_tache_equipe').val()); });
		$('#filtre_tache_permis').off('change').on('change', function() { pmAffectation.controllers.creneau.clickFilter('permis', $('#filtre_tache_permis').val()); });
		$('#filtre_tache_duree').off('change').on('change', function() { pmAffectation.controllers.creneau.clickFilter('duree', $('#filtre_tache_duree').val()); });
		...
		for (var _iCreneau in _creneaux) {
			...
			switch (_creneaux[_iCreneau]['priorite']) {
				case 'orga':
					...
					break;
				case 'equipe':
					if (pmAffectation.data.parameter.equipes[_creneaux[_iCreneau]['equipe']]['nom'].toLowerCase() 
						!= pmAffectation.data.parameter.confiance[_creneaux[_iCreneau]['confiance']]['nom'].toLowerCase()) {
						_priorite = ' <span class="label label-inverse">'+pmAffectation.data.parameter.equipes[_creneaux[_iCreneau]['equipe']]['nom'].toLowerCase()+'</span>';
					}
					break;
				case 'confiance':
			...
			var _html = '<div class="item tache" id="tache_'+_iCreneau+'" idCreneau="'+_creneaux[_iCreneau]['id']+'">'+
						'<span class="badge" style="background-color: '+pmUtils.hexToRgba(_creneaux[_iCreneau]['equipe_couleur'], 0.7)+'">'+
